...
export class PlayerTournamentRunRepository extends Repository<PlayerTournamentRun> {
    ...
    const data = await this.query(
      `
        ...
        SELECT
          ...
          MIN(pt.placement) AS highestPlacement,
          COUNT(DISTINCT pt.tournament_id) AS attendance,
          SUM(CASE WHEN pt.placement = pbp.best_placement THEN 1 ELSE 0 END) AS timesAchievedBestPlacement,
          ARRAY(
            SELECT char
            FROM (
              ...
              GROUP BY char
              ORDER BY usage_count DESC, char  -- Sort by frequency first, then alphabetically as tiebreaker
            ) char_counts
          ) AS charactersUsed
        FROM
          player_tournaments pt
        JOIN
            player_best_placements pbp ON pt.player_id = pbp.player_id
        JOIN
            player_names pn ON pt.player_id = pn.player_id
        GROUP BY
          pt.player_id, pn.consistent_player_name, pn.consistent_profile_image, pbp.best_placement
        ORDER BY
          highestPlacement ASC, timesAchievedBestPlacement DESC, attendance DESC;
        `,
      [eventID],
    );
  ...
  public async findAll(query: FindPlayerTournamentRunsQueryDTO) {
    ...
    if (query.placement) {
      queryBuilder.andWhere('playerTournamentRun.placement = :placement', {
    ...
    // Add sorting
    if (query.sortBy) {
      queryBuilder.orderBy(`playerTournamentRun.${query.sortBy}`, query.order);
    }

    // Add limit
    queryBuilder.take(query.limit);

    const [playerTournamentRuns, total] = await queryBuilder.getManyAndCount();
