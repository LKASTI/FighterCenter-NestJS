...
export class StartggUserRepository extends Repository<StartggUser> {
	...
	public async createAndSave(startggUser: CreateStartggUserDTO): Promise<StartggUser> {
	...
	public async findAll(query: FindStartggUsersQueryDTO) {
		...
		if(query.startggUsername) {
			queryBuilder.andWhere('startggUser.startggUsername = :startggUsername', {
				startggUsername: query.startggUsername,
			});
		}

		if(query.startggGamerTag) {
			queryBuilder.andWhere('startggUser.startggGamerTag = :startggGamerTag', {
				startggGamerTag: query.startggGamerTag,
			});
		}

		if(query.startggUserID) {
			queryBuilder.andWhere('startggUser.startggUserID = :startggUserID', {
