...
export class TournamentDataParserService {
	...
	public async parseStartGGTournamentDataV2(
		...
		try {
			...
			if (params.requestDelay) this.startggRequestDelayMs = params.requestDelay;
			...
			if (params.mustUpdatePlayerProfileImage)
				this.mustUpdatePlayerProfileImage = params.mustUpdatePlayerProfileImage;
			...
			if (!startggSlug || !startggEventSlug)
				throw new BadRequestException(`StartGG Slugs could not be parsed`);
			...
			if (!tournamentData)
				throw new NotFoundException(`Tournament ${startggSlug} not found`);
			...
			if (!tournamentData.events)
				throw new NotFoundException(`No tournaments found for ${startggSlug}`);
			...
			const res = await this.createSets(setsData, newTournament.tournamentID);
	...
	private parseStartGGSetNodeRecord(
		...
		if (set.setGamesType === 1) {
			matches_to_win = Math.floor(set.totalGames / 2) + (set.totalGames % 2);
		}
					...
					for (const selection of game.selections) {
						if (selection.entrant.id === player_one_entrant_id) {
							if (selection.character) {
								...
								if (!player_one_characters.includes(player_one_character))
									player_one_characters.push(player_one_character);
							}
						} else if (selection.entrant.id === player_two_entrant_id) {
							if (selection.character) {
								player_two_character = selection.character.name;
								if (!player_two_characters.includes(player_two_character))
									player_two_characters.push(player_two_character);
							}
						}
					}
				}
	...
	private async queryStartGGEventHasMultipleNames(
		...
		if (eventsData && eventsData.events) return eventsData.events.length > 1;
	}
	//#endregion
	...
	private async createTournament(
		...
		const newTourney = await this.tournamentService.create(createTournamentDTO);
	...
	private async createPlayers(
		...
		for (const set of setsData) {
			...
			const playerOne = {
				...
				playerCountry: set.player_one_country ? set.player_one_country : null,
			...
			const playerTwo = {
				...
				playerCountry: set.player_two_country ? set.player_two_country : null,
			...
			for (const {
				...
				player = await this.playerService.findByStartGGId(playerStartGGId);
				if (!player) {
				...
				if (!playerTournamentRun) {
					playerTournamentRun = await this.playerTournamentRunService.create({
	...
	private async createMatches(
		...
		if (matches.length > 0) {
			for (const match of matches) {
				...
				const newMatch = await this.tournamentMatchService.create(matchQuery);
				// Update stats
	...
	private async createSets(
		...
		for (const set of setsData) {
			...
			const setExistence = await this.tournamentSetService.findByStartGGSetId(
				set.set_id,
			);
			if (setExistence)
				throw new BadRequestException(
			...
			let p1Res = null;
			if (playerOneCharacters !== null && playerOneCharacters.length > 0) {
				p1Res = await this.playerTournamentRunService.updateCharactersUsed(
					playerOne.playerID,
					tournamentID,
					playerOneCharacters,
				);
			...
			if (playerTwoCharacters !== null && playerTwoCharacters.length > 0) {
				p2Res = await this.playerTournamentRunService.updateCharactersUsed(
					playerTwo.playerID,
					tournamentID,
					playerTwoCharacters,
				);
